...
const ScrollStack = ({
  ...
  rotationAmount = 0,
  blurAmount = 0,
  useWindowScroll = true, // Default to true now
  onStackComplete
}: any) => {
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    rotationAmount, blurAmount, useWindowScroll, calculateProgress,
  ...
  useLayoutEffect(() => {
    ...
    // Safely attach to Window Scroll natively
    if (useWindowScroll) {
    ...
    return () => {
      if (useWindowScroll && window.globalLenis) {
    ...
    updateCardTransforms, handleScroll
  ]);
